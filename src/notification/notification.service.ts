...
export class NotificationService{
    ...
    async getNotification(userEmail: string):Promise<any>{
        ...
        const filledSurvey = allSurvey.filter(survey=>filledSurveyIds.includes(survey.id))
        const unfilledSurvey =  allSurvey.filter(survey => !filledSurveyIds.includes(survey.id));       
